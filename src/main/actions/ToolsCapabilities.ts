...
import { runCommandStandalone } from "./Spawner";
...
export async function getProperties(uri: Uri, kind: string): Promise<string> {
  ...
  const info = runCommandStandalone(exe, args, uri.fsPath);
  if (info.error) {
...
export function getSizeFormat(uri: Uri, baseFolder: string): string {
  const info = runCommandStandalone(join(baseFolder, 'avr-size'), ['-h'], uri.fsPath);
  if (info.error) {
